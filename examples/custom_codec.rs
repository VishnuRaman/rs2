...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Keyframe interval: {} frames", custom_config.keyframe_interval);
    println!("  Compression enabled: {}", custom_config.enable_compression);
    ...
    for (i, frame) in test_frames.iter().enumerate() {
        ...
        let mut custom_stream = custom_codec.encode_stream(raw_data_stream, "test-stream".to_string());
        ...
        let mut standard_stream = standard_codec.encode_stream(raw_data_stream, "test-stream".to_string());
    ...
    println!("  Standard codec total size: {} bytes", standard_encoded_size);

    let size_diff_percent = if standard_encoded_size > 0 {
        ((custom_encoded_size as f64 - standard_encoded_size as f64) / standard_encoded_size as f64) * 100.0
    } else {
        0.0
    };
    ...
    println!(
        ...
        if size_diff_percent < 0.0 { "Custom codec" } else { "Standard codec" }
    );
    ...
    println!("  Average compression ratio: {:.2}", custom_stats.average_compression_ratio);
