...
fn test_retry_with_policy_immediate() {
    ...
    rt.block_on(async {
        ...
        assert_eq!(result, vec![
            Ok(1), Err("error1"),  // First attempt
            Ok(1), Err("error1"),  // Second attempt
            Ok(1), Err("error1"),  // Third attempt
        ]);
    });
}
...
fn test_retry_with_policy_fixed_delay() {
    ...
    rt.block_on(async {
        ...
        let policy = RetryPolicy::Fixed { max_retries: 1, delay };
        ...
        assert!(elapsed.as_millis() >= 100, "Expected delay of at least 100ms");
    });
}
