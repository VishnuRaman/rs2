...
async fn test_window_by_time_basic() {
    ...
    let windowed_stream = stream.window_by_time_rs2(config, |event| event.timestamp);
    let results: Vec<TimeWindow<TestEvent>> = windowed_stream.collect_rs2().await;
...
async fn test_window_by_time_empty_stream() {
    ...
    let windowed_stream = stream.window_by_time_rs2(config, |event: &TestEvent| event.timestamp);
    let results: Vec<TimeWindow<TestEvent>> = windowed_stream.collect_rs2().await;
    assert!(results.is_empty());
}
...
async fn test_sliding_count() {
    ...
    let result: Vec<usize> = stream.sliding_count_rs2(3).collect_rs2().await;
...
async fn test_sliding_sum() {
    let data = vec![1, 2, 3, 4, 5];
    let stream = from_iter(data);
    let result: Vec<i32> = stream.sliding_sum_rs2(3).collect_rs2().await;
...
async fn test_sliding_min() {
    ...
    let stream = from_iter(data);
    let result: Vec<Option<i32>> = stream.sliding_min_rs2(3).collect_rs2().await;
...
async fn test_sliding_max() {
    let data = vec![5, 2, 8, 1, 9];
    let stream = from_iter(data);
    let result: Vec<Option<i32>> = stream.sliding_max_rs2(3).collect_rs2().await;
...
async fn test_sliding_window_aggregate() {
    ...
    let result: Vec<String> = stream
        .sliding_window_aggregate_rs2(3, |window| {
...
async fn test_moving_average() {
    ...
    let stream = from_iter(data);
    let result: Vec<f64> = stream.moving_average_rs2(3).collect_rs2().await;
...
async fn test_group_by_time() {
    ...
    let result: Vec<(SystemTime, Vec<TestEvent>)> = stream
        .group_by_time_rs2(Duration::from_secs(60), |event| event.timestamp)
        .collect_rs2().await;
...
async fn test_sliding_window_zero_size() {
    let data = vec![1, 2, 3];
    let stream = from_iter(data);
    let result: Vec<usize> = stream.sliding_count_rs2(0).collect_rs2().await;
...
async fn test_sliding_window_larger_than_data() {
    let data = vec![1, 2];
    let stream = from_iter(data);
    let result: Vec<usize> = stream.sliding_count_rs2(5).collect_rs2().await;
